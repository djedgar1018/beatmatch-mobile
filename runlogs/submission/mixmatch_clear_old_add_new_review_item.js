...
const newReviewSubmissionId = '386173d5-185a-4e65-afc6-8c7fda3ed7dd';
...
(async()=>{
 ...
 out.deleteOldRejectedItem=await step(out,'deleteOldRejectedItem',()=>api('DELETE',`/v1/reviewSubmissionItems/${encodeURIComponent(oldItemId)}`));
 out.createNewItem=await step(out,'createNewItem',()=>api('POST','/v1/reviewSubmissionItems',{
  data:{type:'reviewSubmissionItems',relationships:{reviewSubmission:{data:{type:'reviewSubmissions',id:newReviewSubmissionId}},appStoreVersion:{data:{type:'appStoreVersions',id:versionId}}}}
 }));
 ...
 fs.writeFileSync('runlogs/submission/mixmatch-clear-old-add-new-review-item-result.json',JSON.stringify(out,null,2));
 console.log(JSON.stringify({deleteOldRejectedItem: out.deleteOldRejectedItem===null && !out.errors.find(e=>e.step==='deleteOldRejectedItem') ? '204_or_empty_success' : out.deleteOldRejectedItem, createNewItem: out.createNewItem?.data ? {id:out.createNewItem.data.id,state:out.createNewItem.data.attributes?.state} : null, versionState: out.version?.data?.attributes?.appStoreState, versionBuild: out.version?.data?.relationships?.build?.data, reviewSubmissions: out.after?.data?.map(r=>({id:r.id,state:r.attributes?.state,submittedDate:r.attributes?.submittedDate,items:r.relationships?.items?.data})), included: out.after?.included?.map(i=>({type:i.type,id:i.id,state:i.attributes?.state})), errors: out.errors},null,2));
